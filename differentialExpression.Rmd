...
#Alternative to Effect size is to highlight genes with interaction term
...
plot(res_merged$WT.log2FoldChange,res_merged$hnf1b.log2FoldChange,pch=16,cex=0.5,xlab="WT Salt Induced Log2FC",ylab="Hnf1b Salt Induced Log2FC",col=ifelse(res_merged$`Effect Size` > 1 & abs(res_merged$WT.log2FoldChange) > 1,"red","black"))
res_merged[identify(res_merged$WT.log2FoldChange,res_merged$hnf1b.log2FoldChange,labels=res_merged$mgi),]

ggplot(res_merged,aes(x=WT.log2FoldChange,y=hnf1b.log2FoldChange)) + geom_point(aes(colour = int)) + theme_bw() +
  scale_colour_gradient2("Hnf1b:NaCl\nInteraction\nEffect Size") 

  #scale_colour_manual(values=c("black","red"))+
  annotate("text", x = res_merged["ENSMUSG00000047638",]$WT.log2FoldChange + 1.0, 
           y = res_merged["ENSMUSG00000047638",]$hnf1b.log2FoldChange - 0.2, label = "FXR")

abline(h=0,col="red2")
#res_merged[identify(res_merged$WT.log2FoldChange,res_merged$hnf1b.log2FoldChange,labels=res_merged$mgi),]

res_merged["ENSMUSG00000047638",]
gg_plotCounts("ENSMUSG00000059336")
```
...
```{r}
temp<-res_merged[,c("mgi","WT.baseMean","WT.log2FoldChange","WT.padj","hnf1b.log2FoldChange","hnf1b.padj","Effect Size","Nearby Peak","int","int.padj")]
colnames(temp)<-c("MGI Symbol","baseMean","WT.log2FoldChange","WT.padj","Hnf1b.log2FoldChange","Hnf1b.padj","Effect_Size","Nearby Peak","Interaction","Interaction.Padj")
#Keep Karam's Peaks
#temp<-temp[ft[,1],]
...
idx<-match(f$Gene,rownames(resDF))
f$effect_size<-resDF[idx,]$log2FoldChange
f$Effect<-factor(ifelse(f$effect_size < 0,"Failure to Up Regulate","Failure to Down Regulate"),levels=c("Failure to Up Regulate","Failure to Down Regulate"))

f<-f[with(f,order(effect_size)),]
...
f2<-as.matrix(f[,2:5])
rownames(f2)<-f$label
Heatmap(f2,cluster_columns = FALSE,km=2)

#ha = HeatmapAnnotation(barplot = anno_barplot(f$effect_size))

ha <- HeatmapAnnotation(Effect_Size = row_anno_barplot(f$effect_size, baseline = 0, gp = gpar(fill = ifelse(f$effect_size > 0, "red", "green")),axis = TRUE),which='row',width = unit(2, "cm"),name="Interaction\nEffect\nSize")
